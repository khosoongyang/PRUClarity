    ...
    def _setup_rag(self, pdf_dir: str) -> None:
        try:
            ...
            if not os.path.exists(pdf_dir):
                ...
                logger.info("RAG engine disabled - no documents found")
            ...
            # Create vector index using global settings (no ServiceContext needed)
            self.index = VectorStoreIndex.from_documents(documents)
            self.rag_engine = self.index.as_query_engine(similarity_top_k=3)
            
            logger.info(f"RAG engine initialized from directory: {pdf_dir} with {len(documents)} documents")
